...
* `label` : この行で付けられているラベルを表す文字列。ラベルが無い場合はnull。
* `inst` : この行の命令を表す文字列。命令が無い場合はnull。
...
* `target` : 現在変換に用いる行変換器。未設定の場合はnull。
* `outStart` : 出力を開始するアドレスを表す `BigInt`。未設定の場合はnull。
...
この環境で `BigInt` が使用可能ならtrue、使用不可能ならfalseを返す。
...
* `error` : アセンブルにおいてエラーを検出したかをあラわす真偽値。
